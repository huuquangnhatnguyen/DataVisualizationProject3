...
class VoronoiMap {
  ...
  constructor(selector, config = null) {
    ...
    this.width = this.config.svgWidth - this.config.margin.left - this.config.margin.right;
    this.height = this.config.svgHeight - this.config.margin.top - this.config.margin.bottom;
  ...
  initVis() {
    ...
    this.container = this.drawingArea.append("g")
      .classed("treemap-container", true)
      .attr("transform", `translate(${this.treemapCenter[0]},${this.treemapCenter[1]})`);
    ...
    this.legendContainer = this.drawingArea.append("g")
      .classed("legend", true)
  ...
  updateVis(dataObj) {
    ...
    this.container.select(".total-cost")
      .text("$" + this.totalCost);
    ...
    this.simulation = d3.voronoiMapSimulation(this.data)
      .clip(this.circlingPolygon)
      .weight(d => d.cost)
      .initialPosition(d3.voronoiMapInitialPositionPie().startAngle(-Math.PI/2))
      .on("tick", () => {
  ...
  renderVis() {
    ...
    // Update cell paths
    const cells = this.container.select(".cells")
      .selectAll(".cell")
      .data(this.polygons);
    ...
    // Update cost labels
    const costs = this.container.select(".costs")
      .selectAll(".cost")
      .data(this.polygons);
    ...
    // Update highlighters
    const highlighters = this.container.select(".highlighters")
    ...
    highlighters.enter()
      .append("path")
      .merge(highlighters)
      .attr("class", d => `group-${d.site.originalObject.data.originalData.id} highlighter`)
      .attr("d", d => `M${d.join(",")}z`);
  ...
  updateLegends(data) {
    ...
    const legends = this.legendContainer.selectAll(".legend")
      .data(reversedData)
      .enter();
  ...
  highlight(groupId, shouldHighlight) {
    d3.selectAll(`.group-${groupId}`)
      .classed("highlight", shouldHighlight);
  }
}