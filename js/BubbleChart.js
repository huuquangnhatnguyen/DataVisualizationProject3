class BubbleChart {
    ...
    initVis() {
      ...
      vis.colorScale = d3.scaleOrdinal()
        .domain(['Sheldon', 'Leonard', 'Penny', 'Howard', 'Bernadette', 'Raj', 'Amy'])
    ...
    updateVis() {
      ...
      characterGroups.forEach((bubbles, character) => {
        if (bubbles && bubbles.length > 0) {
          const largest = bubbles.reduce((max, b) => 
            (b.count > max.count) ? b : max, bubbles[0]);
      ...
      vis.simulation.force('x', d3.forceX().strength(0.4).x(d => {
        const center = characterCenters.get(d.character);
        return center ? center.x : vis.width / 2;
      }));
      
      vis.simulation.force('y', d3.forceY().strength(0.4).y(d => {
        const center = characterCenters.get(d.character);
      ...
      vis.simulation
        .nodes(nodes)
        .force('collide', d3.forceCollide().radius(d => d.radius + 0.5).iterations(4))
        .on('tick', () => {
          vis.renderVis();
        })
        .on('end', () => {
      ...
      vis.simulation.force('center-attraction', d3.forceRadial(0, vis.width / 2, vis.height / 2).strength(0.01));
    ...
    renderVis() {
      ...
      const bubblesEnter = bubbles.enter()
        ...
        .attr('class', d => {
          ...
          return `bubble ${largest && d.id === largest.id ? 'largest-bubble' : ''}`;
        })
        .attr('r', d => d.radius)
      ...
      const labels = vis.chart.selectAll('.bubble-label')
        .data(vis.nodes, d => d.id);
      ...
      vis.chart.selectAll('.bubble-label')
        .attr('x', d => d.x)
        .attr('y', d => d.y)
        .style('font-size', d => {
    ...
    createLegend() {
      ...
      // Create legend items
      const legendItems = d3.select('.bubble-legend')
